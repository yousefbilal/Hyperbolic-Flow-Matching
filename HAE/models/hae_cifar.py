...
class HAECifar(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        z_hyp = self.hyperbolic_linear(z_euc)            # (B, feature_size)

        # Hyperbolic classifier
        logits = F.log_softmax(
            self.mlr(z_hyp, self.mlr.c), dim=-1
        )                                                # (B, num_classes)

        # Project back to tangent space at origin and decode
        z_euc_dec = gmath.logmap0(z_hyp, k=self.curvature)  # (B, feature_size)
